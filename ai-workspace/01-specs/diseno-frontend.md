...
1. **Tarjeta de resultado/producto**: logo o avatar a la izquierda; título; **chips/badges** de atributos ("Sin permanencia" → aquí "Plan Text · v1", "ES"); filas de especificación `valor destacado + label pequeño debajo` ("25GB / Datos incluidos" → aquí "42 / Usuarios activos"); **precio grande a la derecha** con sufijo pequeño ("140,25 **€/mes** · IVA incluido"); CTA píldora.
...
- **Estructura**: cabecera con nombre + chips (versión, estado, métricas, divisa); **tabla de tramos por métrica**, siempre en el orden canónico Usuarios → Almacenamiento → Llamadas API — el mismo que los sliders del simulador (patrón 2.4.5, el mismo formato del desglose: "hasta N → precio/unidad", el último "En adelante"); callout si está archivado (*"Este plan ya no se ofrece a clientes nuevos. Los clientes que lo tienen mantienen su tarifa."*); botón «Editar» solo para admin y solo si está activo.
