...
- **Impresión** `(Fase 3, §5.4)`: junto al sello «Guardada» aparece **«Imprimir presupuesto»** → diálogo del navegador. Lo que se imprime no es la pantalla: es una **hoja solo-para-print** (`@media print`, tinta sobre blanco, sin tokens de tema) con membrete, cliente, plan, desglose por tramos, total y pie «Emitido por {nombre} el {fecha} · orientativo, no es una factura». Dos reglas de producto: solo se imprime la simulación **sellada** (el papel lleva el número persistido, nunca un preview) y la divisa de visualización **no se imprime** — el papel va en la divisa de facturación, porque la conversión es una referencia efímera de pantalla. Sin librerías de PDF (→ recortes, roadmap §5.5).
