...
| 24 | El gate de suministro en rojo | `63b2c0e` | 9 altos → 0 sin tocar el umbral: `audit fix`, eslint 9→10 y react-router-dom→react-router 8. Se rechazó el `overrides` de `brace-expansion` porque la v5 exporta `{ expand }` y `minimatch@3` lo llama como función |
