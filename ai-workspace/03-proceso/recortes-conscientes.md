...
| **NIF especiales `K`, `L`, `M`** | Un `L1234567X` se rechaza como inválido, no como "no soportado" | La herramienta es **B2B**: un NIF de menor o de no residente no es cliente corporativo. Sería una rama más en el mismo validador | `../01-specs/features/02-validacion-fiscal-y-alta-cliente.md` §4.4 |
