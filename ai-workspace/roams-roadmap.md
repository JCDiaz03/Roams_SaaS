...
- ✅ **El cron semanal cobró su primera pieza**: el `npm audit` del CI salió en rojo con **9 altos** sobre un repositorio que nadie había tocado — justo el escenario para el que se puso (→ referencia §14.3). Vuelto a **0 sin bajar el umbral**: `npm audit fix` para `fast-uri` y `find-my-way`, **eslint 9 → 10** (la cadena `minimatch@3 → brace-expansion@1`; `eslint.config.js` no necesitó un cambio) y **`react-router-dom` 7 → `react-router` 8**, que no tenía parche en la línea 7 y obligó a migrar los 16 imports. Con él, `engines` sube a **Node ≥ 22.22**. Se rechazó el `overrides` de `brace-expansion` que habría evitado el salto de eslint: la v5 exporta `{ expand }` y `minimatch@3` lo llama como función — habría instalado limpio y roto el linter en ejecución (→ [sesión 24](04-archivo/sesiones/24-el-gate-de-suministro-en-rojo.md))
