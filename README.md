...
**El repositorio se sigue auditando solo, aunque nadie haga push.** Tres vigilancias automáticas: el **CI** corre lint, typecheck, la suite completa y `npm audit` en cada push **y además por cron semanal** — una vulnerabilidad se publica cuando se publica, no cuando hay commits —; **CodeQL** analiza nuestro código (no las dependencias, que ya cubre `audit`) también en push/PR y semanalmente, porque una query nueva puede encontrar un defecto en código que no ha cambiado; y **Dependabot** vigila el lockfile y las GitHub Actions. → [roadmap, «Después de la entrega»](./ai-workspace/roams-roadmap.md).
